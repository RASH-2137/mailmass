  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button
          type="button"
          className="rounded-md bg-zinc-800 px-3 py-1.5 text-sm text-white hover:bg-zinc-700 transition-colors"
        >
          Add Recipients
        </button>
      </DialogTrigger>
    ...
    <div className="flex flex-col flex-1 min-h-0 mt-2 space-y-4">
      <div className="flex items-center gap-3">
        <input
          ...
          className="flex-1 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-300"
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto border border-zinc-800 rounded-md">
        {loading ? (
          <div className="p-4 text-center text-zinc-400">Loading contacts...</div>
        ) : availableContacts.length === 0 ? (
          <div className="p-4 text-center text-zinc-400 bg-zinc-900/50 h-full flex items-center justify-center">
            {search.trim() ? "No unadded contacts found for this search." : "All your contacts are already in this campaign."}
          </div>
        ) : (
          <div className="divide-y divide-zinc-800">
            {availableContacts.map((contact) => (
              <label key={contact.id} className="flex items-center gap-4 px-4 py-3 hover:bg-zinc-800/50 cursor-pointer transition-colors bg-zinc-900">
                <input
                  type="checkbox"
                  checked={selectedContactIds.has(contact.id)}
                  onChange={() => toggleSelection(contact.id)}
                  className="rounded border-zinc-700 bg-zinc-900 text-blue-600 focus:ring-blue-600/50"
                />
                <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded bg-zinc-800 text-white text-sm disabled:opacity-50"
          >
            Previous
          </button>
